...
public sealed class SearchJobsController : ControllerBase
{
    ...
    [FromQuery] int proxyGroupId = 0,
    [FromQuery] Guid? accountId = null,
    CancellationToken ct = default)
    {
        if (proxyGroupId <= 0) return BadRequest("proxyGroupId is required (Option A).");

        var id = await _svc.RunNowAsync(keywordId, priority, proxyGroupId, accountId, ct);
        return Ok(new { jobId = id, priority, proxyGroupId, accountId });
    }
